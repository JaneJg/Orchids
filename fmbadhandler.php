<?php
$FMBH_VERS = "1.18";		// script version

/* ex:set ts=4 sw=4:
 * FormMail PHP Bad Handler.  This script requires PHP 4 or later.
 * Copyright (c) 2004-2005 Open Concepts Pty Ltd t/as Root Software.
 * All rights reserved.
 *
 ...
 *  Note that this must resolve to the IP address that your server will
 ...
 *
 *	By using fmbadhandler.php you agree to indemnify Open Concepts Pty Ltd
 *  Root Software, their agents, employees, and directors from any liability
 *  whatsoever.
 *
 * We still care
 * ~~~~~~~~~~~~~
 ...
 *****************************************************************************/
	...
	//
$TARGET_URLS = array("http://www.nativeorchids.co.nz/");	// default; no URLs allowed
...
if (!isset($aGetVars["bad_state"]))
{
	...
	function GetFormLink(&$s_url)
	{
		...
		$s_url = $s_link = "";
		if (isset($aGetVars) && isset($aGetVars["this_form"]) &&
			isset($sThisScript) && !empty($sThisScript) &&
			CheckValidURL($aGetVars["this_form"]))
		{
				//
	...
	if (isset($aGetVars["bad_template"]) && !empty($aGetVars["bad_template"]))
	{
...
else
{
	...
	if (!CheckValidURL($aGetVars["this_form"]))
		ReportError("Invalid URL '".$aGetVars["this_form"]."'");
